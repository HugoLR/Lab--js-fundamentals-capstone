...
function User(name, role) {
  //Creamos nuestro nuevo objeto con las llaves y valores del usuario
  var userObject = {
    "name": name,
    "role": role,
    "createdAt": Date.now(),

    //Creamos nuestro metódo  que acepta una lista de roles
    hasPermissions(arrayRoles) {
      for(var i = 0; i < arrayRoles.length; i++) {

        //Comparamos la lista de roles con el rol del usuario , para verificar si damos permiso o no
        if (arrayRoles[i] === userObject.role) {
          return true;
        }
      }
      return false;
